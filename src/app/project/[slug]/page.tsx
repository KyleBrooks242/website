...
import { Project, getProjects, getProject } from "../../lib/projectUtil";
...
export async function generateStaticParams() {
  ...
  return projects.map((post: Project) => (
    {
      slug: post.slug,
    }))
}
...
export default async function Blog({ params } : any) {
  ...
  const { project, previous, next } = await getProject(slug);
  ...
  return (
    <section>
      ...
      <div className="grid grid-rows-[20px_1fr_20px] justify-items-center min-h-screen p-8 pb-20 sm:p-20 font-[family-name:var(--font-geist-sans)]">
        ...
        <PostFooter previous={previous} next={next} prefix={'project'}/>
        </main>
      </div>
